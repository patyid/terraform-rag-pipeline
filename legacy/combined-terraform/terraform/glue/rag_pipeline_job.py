...
def _extract_rag_pipeline_app_zip_if_present() -> None:
    """
    Glue adiciona `--extra-py-files` ao sys.path (normalmente como caminho local do .zip).
    Para tornar os imports mais robustos (ex.: namespace packages), extraímos o zip em /tmp
    e colocamos o diretório extraído no início do sys.path.
    """
...
def main() -> None:
    ...
    # Importa do zip enviado via `--extra-py-files` (rag-pipeline-app).
